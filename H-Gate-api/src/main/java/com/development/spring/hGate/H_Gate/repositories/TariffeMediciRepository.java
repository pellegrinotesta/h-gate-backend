...
public interface TariffeMediciRepository extends CrudRepository<TariffeMedici, Integer> {
    ...
    @Query("SELECT t.costo FROM TariffeMedici t WHERE t.medico.id = :medicoId AND t.tipoVisita = :tipoVisita AND t.isPrimaVisita = :isPrimaVisita AND t.isAttiva = true")
    Optional<BigDecimal> findCosto(
            @Param("medicoId") Integer medicoId,
            @Param("tipoVisita") String tipoVisita,
            @Param("isPrimaVisita") boolean isPrimaVisita
    );
